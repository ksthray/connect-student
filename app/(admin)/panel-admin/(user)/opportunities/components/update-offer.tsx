...
const UpdateOffer = ({
  ...
  const { data: entrepries, isLoading: isLoadingCompanies } = useFetch({
  ...
  const companiesData: CompanyType[] = entrepries?.data || [];
  ...
  const companyOptions =
    companiesData?.map((sector) => ({
      label: sector.companyName,
      value: sector.id,
    })) || [];
  ...
  const mutation = useMutation({
    mutationFn: (state: DataPost) => {
      return api.post("/admin/jobs", state, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  if (isLoadingSectors || isLoadingCompanies) {
    return (
      ...
      <DialogContent className="h-[90%] w-full max-w-2xl overflow-y-scroll">
        <DialogHeader>
          ...
          <DialogDescription className="mt-2 text-muted-foreground">
            Modifiez les détails de l&apos;offre d&apos;emploi ci-dessous et
            cliquez sur &quot;Mettre à jour l&apos;offre&quot; pour enregistrer
            les modifications.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="">
              {cover ? (
                ...
                <Label
                  ...
                  Modifier la photo de couverture de cette offre
                </Label>
              )}
              <input
                id="imageUpload"
